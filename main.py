...
def get_address_from_nominatim(lat, lon):
    try:
        ...
        # 這是最關鍵的：檢查 Nominatim 回傳的所有可能欄位
        # 台灣地址的縣市名稱可能在 county, city, state 裡
        # 鄉鎮名稱可能在 town, city_district, suburb, village, district 裡
...
def reverse_geocode(lat, lon):
    try:
        ...
        res = requests.get(url, headers={"User-Agent": "SmartHangerApp/4.0"}, timeout=5)
...
def set_manual(name: str, city: str, town: str, lat: float, lon: float):
    ...
    if (not city or not town) and (lat != 0 and lon != 0):
        city, town = get_address_from_nominatim(lat, lon)
...
def set_by_gps(lat: float, lon: float):
    ...
    CURRENT_LOCATION.update({
        "display_name": f"GPS定位({city}{town})",
        "city": city,
        "town": town,
        "lat": lat,
        "lon": lon
